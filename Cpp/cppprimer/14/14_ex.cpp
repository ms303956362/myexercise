...
public:
    using T = string;
    const T& operator()(bool condition, const T& s_true, const T& s_false) const {
        if (condition)
            return s_true;
        else
            return s_false;
...
public:
    ReadStream(istream& in=cin): is(in){};
...
int main(int argc, char const *argv[])
{
    ...
    // 14.37
    vector<string> sv{"abc", "def","abc", "ghi", "def"};
    std::replace_if(sv.begin(), sv.end(), StrEqual("abc"), "cba");
    for (auto &s : sv)
        cout << s << " ";
    cout << endl;
